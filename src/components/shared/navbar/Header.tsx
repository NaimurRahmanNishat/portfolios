...
const Header = () => {
      ...
      <nav>
        <ul className="hidden lg:flex gap-10 items-center">
          {navLinks.map((link, index) => (
            <li key={index}>
              <Link href={link.path} className={`hover:text-lightGreen transition-all ease-in-out duration-300 relative group font-semibold ${pathname === link?.path && "text-lightGreen"}`}>
                {link.name}
                <span
  ...
  <SheetHeader>
    <SheetTitle className="sr-only">Menu</SheetTitle> {/* <-- add title for a11y */}
  </SheetHeader>

  {/* logo */}
  <div className="mb-36 -mt-8.5 mx-4 text-center text-3xl text-lightGreen">
    <Link href={"/"} onClick={handleClick}>
      <Image src={logo} alt="logo-image" priority className="w-10 h-10"/>
    </Link>
  </div>
    ...
    {navLinks.map((link, index) => {
      return (
        <Link
          ...
          href={link.path}
          onClick={handleClick}
          className={`${link.path === pathname && "text-lightGreen border-b-2 border-lightGreen"} text-xl capitalize hover:text-lightGreen transition-all`}
        >
          {link.name}
        </Link>
      );
